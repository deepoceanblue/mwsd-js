(
  ...
  // const fullTextSearch = document.querySelector('#option-fulltext');
    ...
    //--------------------- vars
    let imCurrent;
    //split result caches for searches with different options 
    //b - any-beginning e - any-end f - fulltext
    let resultCaches = { bef: [], be: [], bf: [], b: [], ef: [], e: [], f: [], };
    //--------------------- event listeners
    startSearchButton.addEventListener('click', function(){
      if(inputField.value.length > 0) { update(inputField.value); }
    });
    ...
    let checkCachedResults = function(userInput, cache) {
      let maxInputHistory = cache.length ;
      let matches = [];
      for (let x = 0 ; x < maxInputHistory ; x++) {  
        if (cache[x][0] === userInput) { 
          cache[x][1].forEach(
    ...
    let convertToHK = function(string) {
      ...
      console.log('converted string:' . ss);
      return ss;
    };

    let sanitize = function(string, imSchemeChars){
      let illegalChars = new RegExp('[^' + allowedChars + imSchemeChars.join('') + ']{1,}');
      let ss = string.trim();
    ...
    let search = function (searchedTerm, searchRange){
      ...
      if (searchRange === 'wordlist') {
        ...
        // st = new RegExp(`\\b${searchedTerm.replace(/\*/g, '.*')}\\b`) ;//FIXME: '\b' operator does not recognizes Unicode characters with diacritics correctly 
        st = new RegExp('[- "\'?&]+' + searchedTerm.replace(/\*/g, '.*') + '[- ,.;"\'?]+', 'u') ;
        console.log(searchedTerm.replace(/\*/g, '.*'));
        ii = 0;
        iiMax = rr.length;
    ...
    let colorizeMatches = function(matches, termToColorize) {
      ...
      for (let ii = 0 ; ii < matches.length ; ++ii){ //colorization regexps, check html attributes for an idea to what they apply
        matches[ii] = matches[ii].replace(/(^[^ \t]+)/g, '<span class="xx">$1</span>'); 
    ...
    let update = function(userInput) {
      ...
      optionAnyBeginningCheckbox.checked && (cacheID = 'b') && (regex.b = '*');
      optionAnyEndingCheckbox.checked && (cacheID += 'e') && (regex.e = '*');
      optionFulltextCheckbox.checked && (cacheID += 'f') && (searchRange = 'fulltext'); 

      console.log(cacheID);
      console.log(resultCaches[cacheID]);
      console.log(typeof resultCaches[cacheID]);
      let inputCached = checkCachedResults(userInput, resultCaches[cacheID]) ;

      if (Array.isArray(inputCached)) {
        displayMatches(inputCached) ;
        console.log(`${userInput} found in cache as: ${inputCached[0]}`) ;
      } else { //if not, look it up in dict, colorize found terms, push into cache and display

        imCurrent = checkOption(imEls);// dear Tom from future: keep it before sanitize() call

        let sanitizedInput = sanitize(userInput, imScheme[imCurrent]);
        ...
        switch (imCurrent){ 
        case 'hk':
          searchedTerm = sanitizedInput ;
          break;
        case 'iast':
          searchedTerm = convertToHK(sanitizedInput);
          break;
        case 'english':
          searchedTerm = sanitizedInput;
          break;
        }
        console.log('imCurrent' + imCurrent);
        console.log('searchedTerm: ' + searchedTerm);
        let termToColorize = searchedTerm.replace(/\*/g, '');
        searchedTerm = regex.b + searchedTerm + regex.e;
        console.log('searchedTerm: ' + searchedTerm);
        ...
        resultCaches[cacheID].push([userInput , matches]);
